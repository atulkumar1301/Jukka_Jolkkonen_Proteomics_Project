...
df <- fread ("~/Library/CloudStorage/OneDrive-UniversityofEasternFinland/Projects/Jukka_Jolkkonen/Jolkkonen_proteomics data/Total/7_Full_Result_Total_GI_Fold_Change.txt")
...
p <- p + geom_hline (aes(yintercept=-log10(0.05), linetype = "p-value 0.05", col="black")) +
  geom_hline (aes (yintercept=-log10(0.00025614754), linetype = "FDR p-value 0.25", col="#D55E00")) +
...
p <- p + scale_x_continuous(breaks = round(seq(-2, 3, by = 1),1))
...
p <- p +
  theme(legend.position="bottom",
        ...
        panel.background = element_blank()) + labs(title=expression("Total-GI"))
p

...
df <- fread ("~/Library/CloudStorage/OneDrive-UniversityofEasternFinland/Projects/Jukka_Jolkkonen/Jolkkonen_proteomics_data/Total/7_Full_Result_Total_tMCAo_Fold_Change.txt")
...
p <- p + geom_hline (aes(yintercept=-log10(0.05), linetype = "p-value 0.05", col="black")) +
  geom_hline (aes (yintercept=-log10(0.00452724358), linetype = "FDR p-value 0.05", col="#D55E00")) +
...
p <- p + scale_x_continuous(breaks = round(seq(-3, 3, by = 1),1))
p <- p + scale_y_continuous(breaks = round (seq (0, 19, by = 2), 1))
